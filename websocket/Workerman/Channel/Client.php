...
class Client
{
	...
	{
		self::connect(/*self::$_remoteIp, self::$_remotePort*/);
		if(!self::$_remoteConnection) return false;
		if (self::$_isWorkermanEnv) {
			return self::$_remoteConnection->send(serialize(array('type' => 'publish', 'channels' => (array)$events, 'data' => $data)));
		} else {
			$body = serialize(array('type' => 'publish', 'channels'=>(array)$events, 'data' => $data));
			$buffer = pack('N', 4+strlen($body)) . $body;
			return fwrite(self::$_remoteConnection, $buffer);
		}
	}
}
